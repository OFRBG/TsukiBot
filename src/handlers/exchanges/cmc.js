...
const buildMessage = (coins, { action, ext }) => {
  ...
  return coins.reduce((msg, coin) => {
    ...
    const bitcoinPrice = `${parseFloat(coinData.price_btc).toFixed(
      8
    )} BTC\` (\`${Math.round(
      parseFloat(coinData.percent_change_24h - bpchg) * 100
    ) / 100}%\`)`;

    const dollarPrice = `${parseFloat(
      coinData.price_usd
    )} USD\` (\`${Math.round(parseFloat(coinData.percent_change_24h) * 100) /
      100}%\`)`;
    ...
    switch (action) {
      case '-':
        return `${msg}${prefix}\` \`${
          ext === 's' ? bitcoinPrice : dollarPrice
        }\n`;
