...
const getPriceCC = (coins, chn, action = '-', ext = 'd') => {
  ...
  cc.priceFull(query.map(c => c.toUpperCase()), ['USD', 'BTC']).then(prices => {
    ...
    const messages = coins.map(coin => {
      ...
      const bitcoinPrice = `${price.BTC.PRICE.toFixed(8)} BTC\` (\`${Math.round(
        price.BTC.CHANGEPCT24HOUR * 100
      ) / 100}%\`)`;
      const dollarPrice = `${price.USD.PRICE} USD\` (\`${Math.round(
        (price.BTC.CHANGEPCT24HOUR + prices.BTC.USD.CHANGEPCT24HOUR) * 100
      ) / 100}%\`)`;
      ...
      switch (action) {
        ...
        case '%':
          try {
            ordered[
              price.BTC.CHANGEPCT24HOUR + prices.BTC.USD.CHANGEPCT24HOUR
            ] = `${prefix}\` \`${ext === 's' ? bitcoinPrice : dollarPrice}`;
            return '';
          } catch (e) {
            console.log('err');
            return '';
